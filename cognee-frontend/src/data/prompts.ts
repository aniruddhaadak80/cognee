...
export const OPENCODE_PROMPT = `You are connected to Cognee Cloud, a persistent knowledge graph memory system. Use it to store and retrieve knowledge across conversations.

## First — is memory already automatic here?

If the Cognee OpenCode plugin or an MCP server is installed, memory works on its own: relevant context is recalled into your prompt each turn, and your turns are captured and converted to long-term memory for you. When that is the case:
- Do NOT call the HTTP API manually (no curl), and do NOT narrate routine recalls/saves.
- Use the provided memory skills/tools only for an explicit deep search or a "remember this permanently" request.
...
export const OPENCODE_SKILLS_CONTENT = `# Cognee Cloud Memory Skill

This skill connects OpenCode to Cognee Cloud for persistent knowledge graph memory.
...
**If these variables are not set**, ask the user to run the export commands from the Cognee Cloud console (Connect to OpenCode → Step 1).

## Session ID — ALWAYS use one

At the start of the conversation, generate ONE id (a fixed prefix "opencode_" plus a unix timestamp, e.g. "opencode_1719320000") and reuse it as \`session_id\` in every call. Sessions group your activity in the Cognee Cloud dashboard and are converted into long-term memory. The ONLY exception: when the user explicitly asks you to store something directly in the knowledge graph, call /remember without a session_id.
...
## Behavior
1. If a Cognee plugin or MCP server is active, memory is automatic — do NOT call the HTTP API manually, and do NOT narrate routine recalls/saves. The remaining guidelines apply only to the HTTP-API fallback.
2. At session start, verify COGNEE_BASE_URL and COGNEE_API_KEY are set; if not, ask the user to export them. Use one session id (agent name + a unix timestamp) as the \`session_id\` in every call — the session appears automatically in the Cognee Cloud dashboard under Sessions and is converted into long-term memory
3. Recall-first: when an answer may depend on earlier context, recall before answering
4. You do NOT need to store after every turn — the session is captured and converted to long-term memory automatically. Store explicitly only for durable facts worth keeping (via /remember/entry with your session_id); use /remember (file upload, no session_id) only when the user explicitly asks to write to the graph
5. Keep memory operations quiet — don't narrate routine recalls or saves
...
// OpenCode marketplace plugin commands — run in a terminal. OpenCode needs hooks
// enabled first, then the Cognee marketplace is registered and the plugin
// installed. See github.com/topoteretes/cognee-integrations/tree/main/integrations/opencode
export const OPENCODE_HOOKS_ENABLE = "opencode features enable hooks";
export const OPENCODE_MARKETPLACE_ADD = "opencode plugin marketplace add topoteretes/cognee-integrations --ref main";
export const OPENCODE_PLUGIN_INSTALL = "opencode plugin add cognee@cognee";
...
export function opencodeSkillInstall(os: PreferredOs): string {
  return writeSkillFile(os, "/.opencode/skills/cognee", "SKILL.md", OPENCODE_SKILLS_CONTENT);
}
